import React, { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { toggleMenu } from "../utils/appSlice";
import { Link } from "react-router-dom";
import { BiSearch } from "react-icons/bi";
import { AiOutlineClose } from "react-icons/ai";
import { youtube_search_api } from "../utils/constant";
import { cacheResults, LRUCache, searchString } from "../utils/searchSlice";

const Head = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const [suggestions, setSuggestions] = useState([]);
  const [autoFocus, setAutoFocus] = useState(false);
  const [showSuggesstion, setShowSuggesstion] = useState(false);
  const searchCache = useSelector((store) => store.search.cache);

  const dispatch = useDispatch();
  const toggleMenuHandler = () => {
    dispatch(toggleMenu());
  };

  //Least recently used caching function
  const LRUCaching = () => {
    const cacheKeys = Object.keys(searchCache);
    if (cacheKeys.length > 10) dispatch(LRUCache(cacheKeys[0]));
  };

  const getSearchSuggesstion = async () => {
    const data = await fetch(youtube_search_api + searchQuery);
    const json = await data.json();
    setSuggestions(json[1]);

    //update cache
    LRUCaching();
    dispatch(cacheResults({ [searchQuery]: json[1] }));
  };

  useEffect(() => {
    // Debouncing-----
    // if we press the key before 2 ms than reconcilliation process happens (due to change in state variables) and thus it will call the
    //  unmounting methods first => clear the interval even befor calling and thus no api call happens.

    const timer = setTimeout(() => {
      if (searchCache[searchQuery]) setSuggestions(searchCache[searchQuery]);
      else getSearchSuggesstion();
    }, 250);

    return () => {
      clearTimeout(timer);
    };
  }, [searchQuery]);

  return (
    <div className=" sticky top-0 bg-white z-50">
      <div className="grid grid-flow-col p-3 py-4 shadow-lg ">
        <div className="flex col-span-1 mt-1">
          <img
            className="h-7 mx-3 cursor-pointer"
            alt="menu"
            src="https://icons.veryicon.com/png/o/miscellaneous/flat-wireframe-library/hamburger-menu-1.png"
            onClick={toggleMenuHandler}
          />

          <a href={"/"}>
            <img
              className="h-7"
              alt="Youtube"
              src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Logo_of_YouTube_%282015-2017%29.svg/2560px-Logo_of_YouTube_%282015-2017%29.svg.png"
            />
          </a>
        </div>

        <div className="col-span-10 text-center relative">
          <div className="">
            <input
              placeholder="Search"
              className="w-1/2 pt-[3px] indent-4 h-[35px] border border-gray-300 rounded-l-full outline-blue-300"
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onBlur={() => {
                setTimeout(() => setShowSuggesstion(false), 250);
              }}
              onFocus={() => {
                setShowSuggesstion(true);
              }}
              autoFocus={autoFocus}
            />
            {suggestions.length > 0 ? (
              <AiOutlineClose
                style={{
                  position: "absolute",
                  right: "17rem",
                  bottom: "0.5rem",
                  cursor: "pointer",
                }}
                onClick={() => {
                  setSearchQuery("");
                  setSuggestions([]);
                  setAutoFocus(true);
                }}
              />
            ) : null}
            <button className=" h-[35px] border border-gray-300 rounded-r-full border-l-0 bg-gray-100 px-3">
              <BiSearch />
            </button>
          </div>
          {showSuggesstion ? (
            <div className="absolute bg-white w-1/2 mt-2 rounded-lg left-[13.5rem] shadow-lg">
              <ul className="text-left">
                {suggestions.map((item, index) => {
                  return (
                    <div
                      onClick={(e) => {
                        setSearchQuery(item);
                        dispatch(searchString(item));
                        // window.scrollTo(0, 0);
                      }}
                      key={index}
                      className="py-1 px-3 shadow-sm hover:bg-gray-100 flex gap-1 "
                    >
                      <BiSearch style={{ margin: "8px" }} /> {item}
                    </div>
                  );
                })}
              </ul>
            </div>
          ) : null}
        </div>
        <div className="">
          <img
            className="h-8 col-span-1"
            src="data:image/png;base64,"
            alt="user"
          />
        </div>
      </div>
    </div>
  );
};

export default Head;
